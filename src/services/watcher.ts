...
import {Feed, PodcastItem, Post, Publisher, Watcher, WatchRequest} from "../types";
import { parse } from "rss-to-json";
import {Stoolie} from "stoolie/dist/logger";


export const createWatcher = (client: any, publisher: Publisher, logger: Stoolie): Watcher => {
    return {
        watch: watch(client, publisher, logger)
    }
}

const watch = (client: any, publisher: Publisher, logger: Stoolie) => async ({ rssFeed, frequencyMs }: WatchRequest): Promise<Subscription> => {
    ...
    return interval(frequencyMs)
        .pipe(
            ...
            RxOp.tap(({ link }) => { showLink = link }),
            RxOp.map(({ items }: Feed) =>  items),
            RxOp.mergeMap((items) => items),
            RxOp.filter(({ published }) => new Date(published).toISOString() > lastReadDate.toISOString()),
            RxOp.tap(({ title, published }) => log.withFields({ title, published: new Date(published).toISOString() }).info("Publishing Post.")),
            RxOp.mergeMap((item: PodcastItem) => publisher.publish({ showLink, item }))
        )
        .subscribe({
            next() {},
            error(error: Error) {
